...
def mean_calculation(dataframe,mean_on,feature):
    if mean_on=='year':
        ...
        for i in years:
            mean_years[i]=round(dataframe.wdsp[(dataframe.yr==i)&(dataframe.wdsp>0)].mean(axis=0,skipna=True),1)
        return mean_years;
    ...
    if mean_on=='stationid':
        ...
        for i in wsids:
            mean_wsids[i]=round(dataframe.stp[(dataframe.wsid==i)&(dataframe.stp>0)].mean(axis=0,skipna=True),1)
        return mean_wsids;
        ...
        for k in stn_yr_mo:
            for l in stn_yr_mo[k]:
                for m in stn_yr_mo[k][l]:
                    if feature=='temp':
                        stn_yr_mo[k][l][m]=round(dataframe.temp[(dataframe.wsid==k)&(dataframe.yr==l)&(dataframe.mo==m)].mean(axis=0,skipna=True),1)
                    if feature=='dewp':
                        stn_yr_mo[k][l][m]=round(dataframe.dewp[(dataframe.wsid==k)&(dataframe.yr==l)&(dataframe.mo==m)].mean(axis=0,skipna=True),1)
...
def wind(dataframe_master):
    dataframe_master.wdsp[dataframe_master.wdsp==0]=round(dataframe_master.wdsp[dataframe_master.wdsp>0].mean(axis=0,skipna=True),1)
    dataframe_master.wdct[dataframe_master.wdct==0]=round(dataframe_master.wdct[dataframe_master.wdct>0].mean(axis=0,skipna=True),1)
    dataframe_master.gust[dataframe_master.gust==0]=round(dataframe_master.gust[dataframe_master.gust>0].mean(axis=0,skipna=True),1)
...
def temp_cleaning(dataframe_master,feature):
    ...
    dataframe_master.temp[(dataframe_master.temp==0)|(dataframe_master.temp.isna())]=round(dataframe_master.temp.mean(axis=0,skipna=True),1)
    return dataframe_master;    
...
def dewp_cleaning(dataframe_master,feature):
    ...
    dataframe_master.dewp[((dataframe_master.dewp==0)|(dataframe_master.dewp.isna()))]=round(dataframe_master.dewp.mean(axis=0,skipna=True),1)
    return dataframe_master;

def hmdy_cleaning(dataframe_master,feature):
    dataframe_master.hmdy[dataframe_master.hmdy==0]=round(dataframe_master.hmdy.mean(axis=0,skipna=True),1)
    return dataframe_master;
...
for i in modes['wsid']:
    ...
    datalog.loc[datalog.wsid == i, 'inme'] = modes.loc[modes['wsid'] == i, 'inme'].to_string()[5:]
    datalog.loc[datalog.wsid == i, 'city'] = modes.loc[modes['wsid'] == i, 'city'].to_string()[5:]
    datalog.loc[datalog.wsid == i, 'prov'] = modes.loc[modes['wsid'] == i, 'prov'].to_string()[5:]
############################################################
...
for i in range(len(datalog.index)):
    if (datalog.loc[i, 'prcp'] == 0) | (datalog.loc[i, 'prcp'] > 60):
        prev = datalog.loc[i-1,'prcp']
        nxt = datalog.loc[i+1,'prcp']
        if (prev > 0) & (np.isnan(nxt)):
            nxt = 0
            avg = (prev + nxt) / 2
            datalog.loc[i,'prcp'] = avg
        elif (np.isnan(prev)) & (nxt > 0):
            prev = 0
            avg = (prev + nxt) / 2
            datalog.loc[i, 'prcp'] = avg
        elif (prev > 0) & (nxt > 0):
...
datalog.loc[datalog['prcp'] == 0, 'prcp'] = np.NaN
################################################################################
